...
def _ivim_error_sums(params, gtab, signal):
    """Error function to be used in fitting the IVIM model.
...
def _ivim_error_sums_reduced(params, gtab, signal, x0):
    """Error function to be used in fitting the IVIM model.
    ...
    #print(f'x0 = {x0}')
    params_full = np.zeros(4)
    params_full[0:1] = x0[0:1]
    params_full[2:] = params
    #print(f'params_full = {params_full}')
    #print(f'params_full = {params_full}')
    residual = signal - ivim_prediction(params_full, gtab)
    residual = np.linalg.norm(residual)
    return residual
...
class IvimModelTRR(ReconstModel):
    ...
    def __init__(self, gtab, split_b_D=400.0, split_b_S0=200., bounds=None,
        ...
        self.voxel_index = 0  # Used for tracker
    ...
    def _leastsq(self, data, x0):
        ...
        maxfev = self.options["maxiter"]
        bounds = self.bounds
        #bounds = [(x0 - x0*0.95, x0 - x0*1.05)]
        #self.bounds
        #print(f'x0 = {x0}')
        #print(f'bounds = {bounds}')
        #print(f'_ivim_error(x0) = {_ivim_error(x0, self.gtab, data)}')
        #print(f'_ivim_error_sums(x0) = {_ivim_error_sums(x0, self.gtab, data)}')

        if 1:  # shgo
            bounds = [(max(x0[0] * 0.95, 0), x0[0] * 1.05),
                      (max(x0[1] * 0.95, 0), min(abs(x0[1] * 1.05), 1.0)),
                      (max(x0[2] * 0.95, 0), min(abs(x0[2] * 1.05), 1)),
                      (max(x0[3] * 0.95, 0), min(abs(x0[3] * 1.05), 1))]
            #print(f'bounds = {bounds}')
            bounds = [(max(x0[0] * 0.95, 0), x0[0] * 3),
                      (max(x0[1] * 0.95, 0), min(abs(x0[1] * 1.05), 1.0)),
                      (max(x0[2] * 0.95, 0), min(abs(x0[2] * 1.05), 1)),
                      (max(x0[3] * 0.95, 0), min(abs(x0[3] * 1.05), 1))]

            bounds = [(max(x0[0] * 0.5, 0), x0[0] * 3),
                      (max(x0[1] * 0.95, 0), min(abs(x0[1] * 5), 1.0)),
                      (max(x0[2] * 0.5, 0), min(abs(x0[2] * 10), 1)),
                      (max(x0[3] * 0.5, 0), min(abs(x0[3] * 10), 1))]

            if 0:
                bounds = [(max(x0[0] * 0.003, 0), x0[0] * 3),
                          (max(x0[1] * 0.00001, 0), min(abs(x0[1] * 10), 1.0)),
                          (max(x0[2] * 0.00001, 0), min(abs(x0[2] * 10), 1)),
                          (max(x0[3] * 0.00001, 0), min(abs(x0[3] * 10), 1))]
                ...
                bounds = [(0.0, 20000.0),
                          ...
                          (0.0, 1.0)]
            if 1:
                #print(f'starting shgo with bounds = {bounds}')
                res = shgo(_ivim_error_sums,
                           bounds=bounds,
                           #n=100,
                           #n=10,
                           n=20,
                ...
                """
                x: array([5.29686901e+03, 2.34090043e-01, 2.97110840e-20, 1.60845095e-03])
                """
            ...
            if 0:
                ...
                shc = SHGO(_ivim_error_sums_reduced, bounds[2:4],
                           args=(self.gtab, data, x0))
                shc.construct_complex()
                print(shc.res)
                print(shc.n)
                print(shc.HC)
                shc.HC.symmetry = None
                shc.iterate()
                # shc.find_minima()
                print(shc.res)
                shc.HC.triangulate()
                shc.HC.V.proc_fpool_nog()
                # shc.HC.recompute_pools(self)
            ...
            #print(f'res.x = {res.x}')
            #print(f'len(res.funl) = {len(res.funl)}')
            #print(f'res.funl = {res.funl}')
            ivim_params = res.x
            #print(f'res.x = {res.x}')
...
class IvimModelVP(ReconstModel):
    ...
    def __init__(self, gtab, bounds=None, maxiter=10, xtol=1e-8):
        ...
        self.voxel_index = -1  # Used to track voxels for plots

        self.f_const = 0.2  # Variable constant for subspace plots
        self.Dstar_const = 7e-9  # Variable constant for subspace plots
    ...
    def fit(self, data, bounds_de=None):
        ...
        #bounds_de = np.array([(0.005, 0.001), (10**-4, 0.001)])
        bounds_de = np.array([(0.005, 0.01), (10 ** -4, 0.001)])
        #bounds1 = np.array([(0.5e-2, 0.01), (1e-6, 1e-3)])
        #bounds1 = np.array([(0.5e-6, 0.01), (1e-6, 1e-3)])
        bounds1 = bounds_de
        ...
        # Initiate the diffusion simplicial complex classes
        if 0:
            shc_dd = SHGO(self.stoc_search_cost, bounds1)
            shc_dd.construct_complex()
            shc_dd.HC.symmetry = None
            #shc_dd.iterate()
            #shc_dd.iterate()
            shc_dd.HC.triangulate()
            shc_dd.HC.refine(30)

        # Initiate the global complex classes
        ...
        # Normal optimization using shgo
        if 1:
            res_one = shgo(self.stoc_search_cost,
                       bounds=bounds1,
                       n=1,
                       args=(data,),
                       sampling_method='sobol',
        ...
        # Reuse complex and compare minima to neigbouring voxels
        elif 0:
            shc_dd.HC.V.fpool = shc_dd.HC.V.cache  # Rea
            print(f'shc_dd.V.fpool = {shc_dd.HC.V.fpool}')
            shc_dd.HC.V.proc_fpool_nog()
        ...
        if 1:
            ...
            bounds = np.array([(1e-4, 0.5), (0.0, 0.01)], )
            bounds = np.array([(1e-6, 0.01), (0.0, 0.001)], )
            shc = SHGO(self.stoc_search_cost, bounds, args=(data,))
            shc.construct_complex()
            shc.HC.symmetry = None
            shc.iterate()
            # shc.find_minima()
            ...
            ax_complex.set_xlabel('$D^*$')
            ax_complex.set_ylabel('$D$', rotation=0)
            ax_complex.set_title('Contour plot of objective function with simplicial complex approximation')
            ax_surface.set_xlabel('$D^*$')
            ax_surface.set_ylabel('$D$', rotation=0)
            ax_surface.set_zlabel('$\epsilon$', rotation=0)
            ax_surface.set_title(
                'Surface plot of objective function with simplicial complex approximation')
        ...
        if 1:
            #bounds = np.array([(0.005, 0.01), (10 ** -4, 0.001)], )
            bounds = np.array([(1e-4, 0.5), (0.0, 0.01)], )
            bounds = np.array([(1e-12, 0.3), (0.001, 0.01)], )
            shc = SHGO(self.const_Dstar_surf, bounds, args=(data,))
            shc.construct_complex()
            shc.HC.symmetry = None
            shc.iterate()
            # shc.find_minima()
            ...
            ax_complex.set_xlabel('$f$')
            ax_complex.set_ylabel('$D$', rotation=0)
            ax_complex.set_title('Contour plot of objective function with simplicial '
                                 'complex approximation with fixed $D^* = 7 \times 10^{-9} $')
            ax_surface.set_xlabel('$f$')
            ax_surface.set_ylabel('$D$', rotation=0)
            ax_surface.set_zlabel('$\epsilon$', rotation=0)
            ax_surface.set_title(
                'Surface plot of objective function with simplicial complex approximation')
        ...
        # Vary f
        if 0:
            f_range = np.linspace(0, 0.5, 120)
            for ind, fri in enumerate(f_range):
                self.f_const = fri
                bounds = np.array([(1e-6, 0.01), (0.0, 0.001)], )
                bounds = np.array([(0.001, 0.05), (1e-12, 0.005)], )
                shc = SHGO(self.const_f_surf, bounds, args=(data,))
                shc.construct_complex()
                shc.HC.symmetry = None
                shc.iterate()
                # shc.find_minima()
                ...
                ax_complex.set_xlabel('$D^*$')
                ax_complex.set_ylabel('$D$', rotation=0)
                ax_complex.set_title(f'Contour plot at f = {fri} of error function')
                ax_surface.set_xlabel('$D^*$')
                ax_surface.set_ylabel('$D$', rotation=0)
                ax_surface.set_zlabel('$\epsilon$', rotation=0)
                ax_surface.set_title(
                    f'Surface plot at f = {fri} of error function')
                ...
                #plt.close()
        #x = res_one.x
        #phi = np.array([0.005, 0.01]) #TODO: DELETE!
        #x = np.array([0.005, 0.01]) #TODO: DELETE!

        # Optimizer #2: Convex Optimizer
        f = self.cvx_fit(data, phi)
        #print(f'cvx_fit_surf(0.2, signal=data, phi): = {cvx_fit_surf(0.2, data, phi)}')
        # plot
        if 0:
            #print(f'self.cvx_fit(x) = {self.cvx_fit)(_}')
            bounds = np.array([0.0, 1.0])
            shc = SHGO(self.cvx_fit, bounds, args=(data,))
            shc.construct_complex()
            shc.HC.symmetry = None
            shc.iterate()
            # shc.find_minima()
        ...
        bounds = [(max(f[0] - 0.05, 0.0), min(f[0] + 0.05, 1.0)),
                  (x[0] * 1e-6, min(abs(x[0] * 10), 1.0)),
                  (x[1] * 1e-6, min(abs(x[1] * 15), 1.0))]

        if 0:
            bounds = [(max(f[0] - 0.05, 0.0), min(f[0] + 0.2, 1.0)),
                      (x[0] * 1e-1, min(abs(x[0] * 10), 1.0)),
                      (x[1] * 1e-4, min(abs(x[1] * 15), 1.0))]

            bounds = [(max(f[0] - 0.005, 0.0), min(f[0] + 0.2, 1.0)),
                      (x[0] * 1e-1, min(abs(x[0] * 10), 1.0)),
                      (x[1] * 1e-4, min(abs(x[1] * 15), 1.0))]

            bounds = [(max(f[0] - 0.005, 0.0), min(f[0] + 0.2, 1.0)),
                      (x[0] * 1e-1, min(abs(x[0] * 100), 1.0)),
                      (x[1] * 1e-4, min(abs(x[1] * 15), 1.0))]

            bounds = [(max(f[0] - 0.005, 0.0), min(f[0] + 0.2, 1.0)),
                      (x[0] * 1e-1, min(abs(x[0] + 0.01), 1.0)),
                      (x[1] * 1e-4, min(abs(x[1] * 15), 1.0))]

        # Optimizer #3: Nonlinear-Least Squares
        #res = least_squares(self.nlls_cost, x_f, bounds=(bounds),
        #                    xtol=self.xtol, args=(data,))
        ## Optimizer #3: least_squares with shgo
        if 1:
            res = shgo(self.nlls_cost,  bounds=bounds, n=20,
                       args=(data,),
                       sampling_method='sobol',
                       #constraints=constraints  #TODO: Must use simplicial
                       )
            result = res.x
        else:
            pass
            #shc = SHGO(f, bounds)
            #TODO: Add x_f, as a point
            #result = x_f
        result = res.x
        f_est = result[0]
        D_star_est = result[1]
    ...
    def const_f_surf(self, x, signal):
        x_f = [self.f_const, x[0], x[1]]

        return self.nlls_cost(x_f, signal)

    def const_Dstar_surf(self, x, signal):
        x_f = [x[0], self.Dstar_const, x[1]]
        return self.nlls_cost(x_f, signal)
    ...
    def cvx_fit_surf(self, f, signal, phi):
        """
        Surface of cvx_fit
        """
